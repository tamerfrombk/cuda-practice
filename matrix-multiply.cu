...
// This is a CUDA kernel designed to multiply two input matrixes using the
// simplest approach:
// 1. square matrix
...
inline void run_simple_matrix_multiply(int n, int dim, dim3 threadsPerBlock,
                                       dim3 blockConfig) {
  cuda_context ctx;
...
// This is a CUDA kernel designed to multiply two input matrixes using tiling.
...
inline void run_tiled_matrix_multiply(int n, int dim, dim3 threadsPerBlock,
                                      dim3 blockConfig) {
  cuda_context ctx;
  auto hm = ctx.allocate_host_memory(n);
  for (int i = 0; i < n; i++) {
...
int main() {
  print_cuda_properties();
  {
    ...
    run_simple_matrix_multiply(n, dim, threadsPerBlock, blockConfig);
  }
  {
    int dim = 256;
    int n = dim * dim;
    ...
    run_tiled_matrix_multiply(n, dim, threadsPerBlock, blockConfig);
  }
}
